...
function LanguageSwitcher() {
  ...
  const switchLang = (newLocale) => {
    // Простой способ: перезагружаем страницу с новым префиксом языка
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-start px-4 py-8 bg-slate-950 text-white">
      <div className="max-w-5xl w-full space-y-8">
        {/* Header with Logo, Auth, Language Switcher */}
            ...
            {!session ? (
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {authLoading ? t('login.sending') : t('login.button')}
                </button>
              </div>
            ) : (
              <div className="flex items-center gap-3">
          ...
          <div className="w-full max-w-3xl mx-auto bg-slate-800 border border-slate-700 rounded-2xl p-6 shadow-xl animate-in fade-in">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold flex items-center gap-2">
                <Zap className="w-5 h-5 text-amber-400" /> {t('liveExample.title')}
              </h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div className="p-3 bg-slate-900 rounded-lg text-center">
                <p className="text-xs text-slate-400">{t('liveExample.geoScore')}</p>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowPricing(false)}>
            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 w-full max-w-md shadow-2xl animate-in fade-in zoom-in" onClick={(e) => e.stopPropagation()}>
              <div className="text-center space-y-4">
                ...
                >
                  {t('upgrade.subscribeButton')} <ArrowRight className="w-4 h-4" />
                </button>
        ...
        {session && showHistory && (
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 animate-in fade-in slide-in-from-top">
            ...
            {history.length === 0 ? (
              <p className="text-slate-400">{t('history.noScans')}</p>
            ) : (
            ...
            <div className="p-8 bg-slate-800 border border-slate-700 rounded-2xl text-center space-y-6 relative overflow-hidden">
              ...
              <div className="relative">
                <h2 className="text-2xl font-semibold mb-2 flex items-center justify-center gap-2">
                  <Zap className="w-6 h-6 text-emerald-400" /> {t('results.geoScore')}
                </h2>
                ...
                <p className="text-slate-400 mt-2">
                  {t('results.average')}
                  <br />
                  {results.totalScore < 50 ? t('results.improvements') : t('results.onTrack')}
                </p>
                <div className="flex flex-wrap justify-center gap-4 mt-6">
                  ...
                  {profile?.subscription_status === 'active' ? (
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        <Wand2 className="w-4 h-4" /> {t('results.aiFixesPro')}
                      </button>
                      <button
                  ...
                  >
                    <Swords className="w-4 h-4" /> {t('results.compareCompetitors')}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setChatOpen(false)}>
            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-lg h-[500px] flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold flex items-center gap-2"><MessageSquare className="w-5 h-5" /> {t('aiAssistant.title')}</h3>
              ...
              <div className="flex-1 overflow-y-auto space-y-2 mb-4">
                ...
                {chatLoading && <div className="text-slate-400 text-sm">{t('aiAssistant.thinking')}</div>}
              </div>
              <div className="flex gap-2">
                <input
                  type="text"
                ...
                />
                <button onClick={handleChatSend} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 rounded-lg text-white">{t('aiAssistant.send')}</button>
              </div>
            </div>
          </div>
        )}
              ...
              {['robots', 'llms', 'schema'].map(type => (
                <div key={type} className="mb-4">
                  ...
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => navigator.clipboard.writeText(fixesData[type])} className="px-3 py-1 bg-blue-600 rounded text-sm flex items-center gap-1"><Copy className="w-3 h-3" /> {t('aiFixes.copy')}</button>
                    <button onClick={() => {
                      ...
                      a.click();
                    }} className="px-3 py-1 bg-emerald-600 rounded text-sm flex items-center gap-1"><Download className="w-3 h-3" /> {t('aiFixes.download')}</button>
                  </div>
                </div>
              ))}
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {competitors.map((comp, i) => (
                  <input
                    key={i}
                    type="url"
                    placeholder={t('compare.placeholder', { number: i + 1 })}
                  ...
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="border-b border-slate-700">
                        <th className="py-2 px-3">{t('compare.module')}</th>
                        <th className="py-2 px-3">{t('compare.yourSite')}</th>
                    ...
                    <tbody>
                      ...
                      <tr className="border-t-2 border-slate-600">
                        <td className="py-2 px-3 font-bold">{t('compare.geoScore')}</td>

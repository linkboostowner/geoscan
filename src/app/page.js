...
export default function Home() {
    ...
    try {
      ...
      if (session) {
        await supabase.from('scans').insert({
          user_id: session.user.id, url, total_score: data.totalScore,
          robots_score: data.robots.score, llms_score: data.llms.score,
          sitemap_score: data.sitemap.score, meta_score: data.meta.score,
          og_score: data.openGraph.score, schema_score: data.schema.score, details: data,
        });
        fetchHistory(session.user.id);
      }
  ...
  const handleUpgrade = async () => { /* ... (без изменений) ... */ };
  const handleOneTimePayment = async () => { /* ... (без изменений) ... */ };
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-start px-4 py-8 bg-slate-950 text-white">
      <div className="max-w-5xl w-full space-y-8">
        ...
        {/* ... (точно такой же JSX, как в предыдущей рабочей версии, но без блока LLM Crawlers и без блока Priorities в результатах) ... */}
      </div>
      {/* AI Chat Modal, AI Fixes Modal, Compare Modal */}
    </main>
  );
}